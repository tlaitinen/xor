...
namespace up {
    ...
    inline  std::string litSetName(const std::vector<Lit>& lits) {
          std::set<std::string> ints;
          for (size_t i = 0; i < lits.size(); i++) {
              Lit l = lits[i];
              ints.insert((l.get_sign() ? "-" : "") + bx::toString(l.get_var()));
...
//              trailPos[p.get_var()] = trail.size();
          ...
          void removeClause(Clause* c) {
              if (settings.learnAll && c->learnt) {
                  std::set<int> n;
                  const std::vector<Lit>& explanation = c->lits;
                  for (size_t i = 0; i < explanation.size(); i++) {
                      Lit l = explanation[i];
                      n.insert(l.get_var()*2 + (int) l.get_sign());
                  }
                  existing.erase(n);
              }
#ifdef TRACK_DUPLICATES
        ...
        public:
          ...
          void reduceDB()
          {
              ...
              for (i = 0; i < learnts.size(); i++) {
                  if (locked(*learnts[i]))
                      numLocked++;
                  if (learnts[i]->lits.size() == 2)
                      numSmall ++;
              }
              std::cout << numLocked << " locked " << numSmall << " binary xor-clauses" << std::endl;
              ...
              std::cout << "removed " << learnts.size() - j << std::endl;
              learnts.resize(j);
          ...
          XorModule() : trailHead(0), cla_inc(1), clause_decay(1 / 0.999), nof_learnts(1000) {
              ...
              settings.xorLearn = 0;
              settings.deepCuts = false;
              settings.firstUipCuts = false;
              settings.learnAll = false;
          }
...
//                    return -v;
        ...
        public:
            ...
            bool hasClauses() const {
                return !clauses.empty() || !learnts.empty();
            }
            bool propagate() {
...
//                    assumptions.push_back(Lit(var, val==false));
            ...
            void explain(Lit lit, std::vector<Lit>& explanation)  {
                if (numExplained[lit.get_var()] != 0) {
                    ...
                    if (settings.deepCuts || settings.firstUipCuts) {
...
//                        std::string n = litSetName(explanation);
                        std::set<int> n;
                        for (size_t i = 0; i < explanation.size(); i++) {
                            Lit l = explanation[i];
                            n.insert(l.get_var()*2 + (int) l.get_sign());
                ...
                verify(explanation);
#endif
                stats.deepExpLits += explanation.size();
