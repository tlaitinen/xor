...
void Solver::forceCheckConsistency() {
    const std::vector<bx::XorModule::Lit>& implied = *eqModule.get_implied_lits();
    bool ignore = false;
    for (size_t i = 0; i < implied.size(); i++) {
        bx::XorModule::Lit l = implied[i];
        Var v = l.get_var() - 1;
        Lit ll = Lit(v, l.get_sign());
        if (ll == xorConflictLit)
            ignore = true;

    if (!((value(v) == l_True && l.get_sign() == false) || (value(v) == l_False && l.get_sign() == true)
                || ll == xorConflictLit || ignore)) {
  //              || value(v) == l_Undef))  {
        reportf("EQModule: \n%s\n", eqModule.toString().c_str());;
        forcePrintState();
        reportf("xor implied (bx indices): %s\n", bx::toString(implied).c_str());
        reportf("value of x%d disagrees (conflict var %d)\n", l.get_var(), var(xorConflictLit));
        exit(1);
    }
        
    
        assert((value(v) == l_True && l.get_sign() == false) 
                || (value(v) == l_False &&  l.get_sign() == true)
//                || (value(v) == l_Undef)
                || ll == xorConflictLit
                || ignore);
    }
...
void Solver::forcePrintState() {
    ...
    for (int i = 0; i < trail.size(); i++) {
        ...
        if (xorJustifiable[v])
            rs = " (xor" + bx::toString((int)xorJustifiable[v]) + ")";
    ...
    reportf("    %4s%4s\n", 
                (xorTrail.size()==trail.size()) ? "X>" : "  ",
                qhead==trail.size() ? "->" : "  ");
    reportf("XORTRAIL------------------------------------------------------\n");
    for (int i = 0; i < xorTrail.size(); i++) {
        Lit l = xorTrail[i];
        string rs;
        Var v = var(l);
        Clause* c = reason[v];
