...
void XorModule::removeDuplicates(std::vector<Lit>& lits, bool evenElim) {
   ...
   void XorModule::addPendingImplied(int untilVar)
        ...
        while (1) {
            ...
            if (!assigns.empty()) {
                ...
                if  (top.var == vr.var) {
                    ...
                    if (parity != oldParity) {
                        ...
                        if (vars[v].r1 != 0) {
                            ...
                            if (settings.fastExp) {
                                fud.explain(conflict, vars[v].r1, vars[v].r2, stats.evenParityEliminated);
                            } else {
                ...
                if (a.r1) {
                    ...
                    if (!vars[v].internal) {
                        D(dbg::prop, "implied literal " << bx::toString(a.lit));
                        implied_lits.push_back(a.lit);
                    }
                    else {

                        D(dbg::prop, "implied internal " << bx::toString(a.lit));
                        implied_internals.push_back(v);
                    }
                    if (a.r2) {
                        // if the reason has two terms, then it is a ternary ->
            ...
            if (!commonPairs.empty()) {
                ...
                for (set<int>::iterator i = varsLeft.begin(); 
                    ...
                    if (part.size() == 2) {
                        ...
                        stats.optNewVars++;
//                        vars[extra].internal = true; TODO: xor-conflict may still return these, fix
...
void XorModule::explainDeep(std::vector<Lit>& explanation) {
    ...
    if (settings.evenParityElimination) {
        for (size_t i = 0; i < explanation.size(); i++) {
            ...
            vars[v].expParity++;
            D(dbg::prop, "expParity " << v << " = " << vars[v].expParity);
...
/*            for (size_t i = 0; i < explanation.size(); i++) {
            ...
            pendingXorsList.push_back(vector<Lit>());
            std::copy(explanation.begin(), explanation.end(),
                    back_inserter(pendingXorsList.back()));
//            pair<PendingXors::iterator,bool> i = 
//                pendingXors.insert(make_pair(varSet, rhs));
...
//                D(dbg::parexp, "Adding pending xor " 
//                        << bx::toString(varSet)
//                        << " = " << rhs);
                D(dbg::parexp, "parity explanation: " << bx::toString(explanation));
//                fud.toGraphviz(bx::toString(stats.learntClauses) + "fud.dot");
...
#endif
        D(dbg::prop, "fud.unionCount " << fud.unionCount());
