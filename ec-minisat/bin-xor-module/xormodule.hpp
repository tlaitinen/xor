...
public:
    ...
    typedef std::vector< std::vector<Lit> > PendingXorsList;
    
private:
    PendingXors pendingXors;
    PendingXorsList pendingXorsList;
...
//    std::deque<int> toExplain;
    ...
    bool initialized;
        ...
        void explainDeep(std::vector<Lit>& explanation);
    void removeDuplicates(std::vector<Lit>& lits, bool evenElim);

    void addPendingImplied(int untilVar);
    ...
    const PendingXorsList& getPendingXors() const {
        return pendingXorsList;
    }

    void clearPendingXors() {
        pendingXorsList.clear();
    }
