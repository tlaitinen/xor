...
namespace xorsat {
    ...
    void ImplicationGraph::dfsExplain(NodePos n) {
        ...
        if (node.varId && (!deepCut || 
                    (node.parent1 == PremiseNode && node.parent2 == PremiseNode))) {
            if (!reason.has(n)) {
                if (n < firstReasonNode)
                    firstReasonNode = n;
                reason.add(n);
    ...
    void ImplicationGraph::justify(Disjunction& d, 
        ...
        if (!firstUIP && !evenElim) {
            ...
            while (lastLvl > 0 && target < dLevels[lastLvl])
                lastLvl--;
            ...
            while (numToExplain > 0)
            {
                ...
                if (toExplain[n] % 2 == 0) {
                    if (evenElim) {
                        evenParityEliminated++;
                        numEliminated++;
                        DBG(1, "eliminated " << n << " " << (int)(toExplain[n]) << " occurrences");
                        reason.remove(n);
                        if (nodes[n].varId > 0) {
                              DBG(1, "eliminated participated variable x"
    
                             << nodes[n].varId);

                            participated.push_back(nodes[n].varId);
                        }
                        numToExplain--;
                        toExplain[n] = 0;
                        continue;
                ...
                if (node.varId == 0 || 
                        (deepCut && (node.parent1 != PremiseNode
                                 || node.parent2 != PremiseNode))) {
