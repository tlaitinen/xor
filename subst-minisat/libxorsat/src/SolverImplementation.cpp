...
//        r.push_back("  Decision level: " + xorsat::toString(decisionLevel));
    ...
    void SolverImplementation::addPendingXor(const Disjunction& d) {
        ...
        for (size_t i = 0; i < d.literals.size(); i++) {
            varSet.insert(d.literals[i].variable);
            if (d.literals[i].negative)
                rhs = !rhs;
        }
        ...
        if (i.second) {
            string s;
            for (set<int>::iterator i2 = varSet.begin(); i2 != varSet.end();
                    i2++) {
                s += " " + xorsat::toString(*i2);
            }

            DBG(1, "adding pending xor" << s << " = " << rhs);
        }
    }
    void SolverImplementation::explain(Disjunction& d){
        ...
        implGraph.justify(d, np, settings.firstUIPCuts, 
                          ...
                          settings.evenParityElimination,
                          settings.deepCuts,
                          numEliminated);

        if (numEliminated && d.literals.size() < settings.xorLearn)
            addPendingXor(d);
    ...
    const vector<Literal>& SolverImplementation::getImplications() const {
    ...
    void SolverImplementation::justify(Disjunction& impl,
        ...
        implGraph.justify(impl, 
                          ...
                          settings.deepCuts,
                          numEliminated);
        if (impl.literals.size() < settings.xorLearn && numEliminated)
            addPendingXor(impl);
    ...
    void SolverImplementation::addPendingXors() {
        if (undoStack.size() > 0)
            throw Error("Cannot add xor-clauses after assignments");
        for (PendingXors::const_iterator i = pendingXors.begin(); 
                i != pendingXors.end(); i++) {
            const pair<set<int>, bool>& pxor = *i;
            vector<int> xorClause;

            for (set<int>::iterator i2 = pxor.first.begin(); 
                    i2 != pxor.first.end(); i2++) {
                xorClause.push_back(*i2);
            }
            if (!pxor.second)
                xorClause.push_back(0);
            addClause(xorClause);
        }
        pendingXors.clear();
    ...
    void SolverImplementation::verify(Disjunction& d) {
        unsigned int bt = simplex.set_backtrack_point();
        bool unsat = false;
        for (int i = 0; i < d.literals.size(); i++) {
            Literal& l = d.literals[i];
            if (simplex.assume(l.variable, l.negative) == false) {
                unsat = true;
                break;
            }
        }
        if (!unsat) {
            ...
            cout << "Clause ok: " << d.toString() << endl;
        }
